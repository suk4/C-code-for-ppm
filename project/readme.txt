Readme for COE3DQ5 Project Software Model 2014
Prepared by Jason Thong and Nicola Nicolici
...
been tested with gcc in Linux (RedHat and Ubuntu) and Mac OS, and Borland in Windows. Unless there are something unforeseen settings, the sources should compile and run on other platforms.
...
(Google for it) is a freeware viewer for Windows/Mac/Linux. MATLAB can also be used.
...
encode_all:
This encodes the .ppm file to a .mic8 file (McMaster Image Compression, revision 8).
...
decode_m3:
This is a reference for milestone 3 in hardware. The .mic8 file is lossless decoded
...
The script process_all.sh compiles all the C files and it runs all the executables 
(in the same order as the data flow given above) on all the bmp files in the working
directory; subsequently, one can use the corresponding debug files as needed by the
milestone that is currently implemented. The script clean_all.sh deletes all the
executables (assuming they are named as the C files) and the debug files produced by
process_all.sh - hope this helps.
