...
//decoding of milestone 3 - read a .mic8 file, do lossless decoding, write a .sram_d2 file
...
int main(int argc, char **argv) {
	...
	if (argc<2) {
		printf("enter the input file name including the .mic8 extension: ");
		// gets(input_filename);
	...
	for (color=0; color<3; color++) {
		...
		for (i=0; i<height; i+=8) for (j=0; j<width_temp; j+=8) {	//which block of 8x8, i and j is the x and y location of the top-left pixel in the block
			for (k=0; k<64; k++) {
				...
				if (m==0) {	//need to read more
					if (read_bits(file_ptr, 1)==1) {	//001 = long (9 bit) coefficient
						n = read_bits(file_ptr, 9);
						decoded_data[zigzag_order[k]] = (n >= 256) ? (n-512) : n; 
						
						/////////////////////////////////////////////
						// if ((color==0) && (i==0) && (j==0)) printf("%d: %d\n", k, decoded_data[zigzag_order[k]]);
						/////////////////////////////////////////////

					}
					else {	//need to read more
						if (read_bits(file_ptr, 1)==1) {	//0001 = three short coefficients
							n = read_bits(file_ptr, 4);
							decoded_data[zigzag_order[k]] = (n >= 8) ? (n-16) : n; //sign extension
							
							/////////////////////////////////////////////
							// if ((color==0) && (i==0) && (j==0)) printf("%d: %d\n", k, decoded_data[zigzag_order[k]]);
							/////////////////////////////////////////////
							
							if (k==63) { printf("unexpected error in bitstream, triple 3-bit coefficients detected at end of block, exiting...\n"); exit(1); }
							k++;
							n = read_bits(file_ptr, 4);
				...
				else if (m==1) {	//01 = run of zeros not to end
					n = read_bits(file_ptr, 4);
				...
				else if (m==2) {	//10 = two short coefficients
					n = read_bits(file_ptr, 4);
					decoded_data[zigzag_order[k]] = (n >= 8) ? (n-16) : n;	//sign extension
				...
				else {				//11 = a single short coefficient
					n = read_bits(file_ptr, 4);
					decoded_data[zigzag_order[k]] = (n >= 8) ? (n-16) : n;
