//coded by Jason Thong for COE3DQ5
//encoding of image from .ppm to .mic8
...
int main(int argc, char **argv) {
	...
	if (argc<3) {
		printf("enter the output file name including the .mic8 extension: ");
...
0000: zero to end
0001: three short coefficients, each 4 bits
001x: a single long coefficient, 9 bits
01xx: run of zeros not to end, 4 bits to say number of zeros (1 to 16 inclusive)
10xx: two short coefficients, each 4 bits
11xx: a single short coefficient, 4 bits
*/
		...
		for (i=0; i<height; i+=8) for (j=0; j<width_temp; j+=8) {
			...
			for (k=0; k<64; k++) {
				if (quantized[zigzag_order[k]]==0) {
					...
					if (n==64) {
						write_bits(file_ptr, 0, 4);		//0000 - run of zeros to end header
						...
						m=0;
						write_bits(file_ptr, 1, 2);		//01 - consecutive zeros header
					...
					if (quantized[zigzag_order[k]] < 8 && quantized[zigzag_order[k]] >= -8) {	//short coefficient, representable on 4 bits
						if (k+1<64 && quantized[zigzag_order[k+1]] < 8 && quantized[zigzag_order[k+1]] >= -8) {	//a second short coefficient, could be zero
							if (quantized[zigzag_order[k+1]]==0 && k+2<64 && quantized[zigzag_order[k+2]]==0) {	//2 zeros after first coefficient, write it alone
								write_bits(file_ptr, 3, 2);										//11 - single short coefficient header
								write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the 4 bits
							}
							else if (k+2<64 && quantized[zigzag_order[k+2]] < 8 && quantized[zigzag_order[k+2]] >= -8) {	//a third short coefficient
								if (quantized[zigzag_order[k+2]]==0 && k+3<64 && quantized[zigzag_order[k+3]]==0) {	//2 zeros after second coefficient, write two only
									write_bits(file_ptr, 2, 2);										//10 - double short coefficient header
									write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the first 4 bits
									k++;
									write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the last 4 bits
								}
								else {	//even if next coefficient is zero, so long as it's not part of a run of zeros, encode it together (all 3 coefficients)
									write_bits(file_ptr, 1, 4);										//0001 - triple short coefficient header
									write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the first 4 bits
									k++;
									write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the middle 4 bits
									k++;
									write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the last 4 bits
								}
							}
							else {	//2 short coefficients so far, not followed by a third one (and thus not followed by a zero), write two coefficients together
								write_bits(file_ptr, 2, 2);										//10 - double short coefficient header
								write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the first 4 bits
								k++;
								write_bits(file_ptr, quantized[zigzag_order[k]] & 0xf, 4);		//write the last 4 bits
						...
						write_bits(file_ptr, 1, 3);										//001 - long coefficient header
